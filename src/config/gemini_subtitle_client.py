...
"""
    ...
    def _fix_subtitle_timing(self, srt_content: str) -> str:
        ...
        # Filtrar legendas válidas e ordenar por tempo de início
        valid_subtitles = []
        for subtitle in subtitles:
            if "start" in subtitle and "end" in subtitle and "text" in subtitle:
                start_time = time_to_seconds(subtitle["start"])
                end_time = time_to_seconds(subtitle["end"])
                subtitle["start_seconds"] = start_time
                subtitle["end_seconds"] = end_time
                valid_subtitles.append(subtitle)
        
        # Ordenar por tempo de início
        valid_subtitles.sort(key=lambda x: x["start_seconds"])
        
        # Redistribuir timing sequencialmente para evitar sobreposições
        MIN_DURATION = 2.0  # Duração mínima de 2 segundos
        GAP_BETWEEN = 0.3   # Gap de 300ms entre legendas
        
        current_time = 0.0
        
        for i, subtitle in enumerate(valid_subtitles):
            # Calcular duração desejada baseada no comprimento do texto
            text_length = len(subtitle["text"].replace('\n', ' '))
            # Duração baseada em velocidade de leitura (aproximadamente 4 caracteres por segundo)
            desired_duration = max(MIN_DURATION, text_length / 4.0)
            # Limitar duração máxima para evitar legendas muito longas
            desired_duration = min(desired_duration, 6.0)
            
            # Definir novo tempo de início
            new_start = current_time
            new_end = new_start + desired_duration
            
            # Atualizar timestamps
            subtitle["start"] = seconds_to_time(new_start)
            subtitle["end"] = seconds_to_time(new_end)
            subtitle["id"] = i + 1  # Renumerar sequencialmente
            
            # Próxima legenda começará após gap
            current_time = new_end + GAP_BETWEEN
            
            print(f"     - Legenda {i+1}: {subtitle['start']} --> {subtitle['end']} ({desired_duration:.1f}s)")
        
        # Usar apenas as legendas válidas redistribuídas
